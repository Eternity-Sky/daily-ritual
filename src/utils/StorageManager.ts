...
export class StorageManager {
    private static readonly CHECK_IN_KEY = 'daily-ritual.check-ins';
    private static readonly SETTINGS_KEY = 'daily-ritual.settings';
    private static readonly STATS_KEY = 'daily-ritual.stats';

    constructor(private globalState: vscode.Memento) {}

    // 获取所有打卡记录
    getCheckInRecords(): CheckInRecord[] {
        return this.globalState.get<CheckInRecord[]>(StorageManager.CHECK_IN_KEY, []);
    }
    ...
    // 获取今天的打卡记录
    getTodayCheckIn(): CheckInRecord | undefined {
        const today = new Date().toISOString().split('T')[0];
        const records = this.getCheckInRecords();
    ...
    getUserStats(): UserStats {
        return this.globalState.get<UserStats>(StorageManager.STATS_KEY, {
            totalCheckIns: 0,
            currentStreak: 0,
            longestStreak: 0,
            lastCheckInDate: ''
